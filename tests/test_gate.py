...
def make_gate(monkeypatch):
    ...
    def make(door=0, uiid=216):
        params = {} if door is None else {"doorState": door}
        reg, entities = init({"extra": {"uiid": uiid}, "params": params})
...
def test_direction_reversal_waits_for_stop_acknowledgement(
    make_gate, loop, initial, target
):
    reg, gate = make_gate(1)
...
def test_new_request_supersedes_a_pending_reversal(make_gate, loop, new_command):
    reg, gate = make_gate(1)
    command(reg, "off", "closing")
...
def test_interrupted_reversal_never_restarts_the_gate(make_gate, loop, interruption):
    reg, gate = make_gate(1)
    command(reg, "off", "closing")
...
def test_stop_does_not_wait_for_a_movement_acknowledgement(make_gate, loop):
    reg, gate = make_gate()
...
def test_new_movement_waits_for_an_explicit_stop_acknowledgement(make_gate, loop):
    reg, gate = make_gate(1)
...
def test_uiid216_tracks_movement_without_device_configuration(make_gate):
    reg, gate = make_gate()
...
def test_existing_transport_callers_keep_their_return_contract(make_gate, loop, status):
    reg, gate = make_gate()
...
def test_gate_position_control_remains_unsupported(make_gate):
    reg, gate = make_gate()
    reg.call(gate.async_set_cover_position(50))
    reg.cloud.send.assert_not_called()
    assert_gate(gate, "closed", "closed", False)
